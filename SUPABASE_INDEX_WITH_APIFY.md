...
Deno.serve(async (req)=>{
  try {
    ...
    if (path.endsWith("/places-autocomplete") && req.method === "POST") {
      ...
      try {
        const res = await fetch(`https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(input)}&types=address&components=country:us&key=${apiKey}`);
      ...
      if (rentCastApiKey) {
        try {
          ...
          if (rentCastRes.ok) {
            ...
            const rentCastData = Array.isArray(rentCastResponse) && rentCastResponse.length > 0 ? rentCastResponse[0] : rentCastResponse && typeof rentCastResponse === 'object' ? rentCastResponse : null;
            if (rentCastData) {
              listingData = {
...
5. RISK FACTORS:
   Identify what's preventing this property from selling faster. For each risk factor, provide a psychologically-informed explanation that helps the user understand why it matters and how it impacts their situation. Frame each risk with empathy and insight, like a trusted advisor sharing perspective.
   
   Each risk factor should:
   - Be specific and data-driven (reference actual numbers from listing data when available)
   - Include a 1-2 sentence micro-explanation that explains why this risk matters and how it can influence outcomes
   - Use human, non-generic language—avoid clichés or robotic phrasing
   - Create curiosity and intrigue—hint at how small changes could improve outcomes without revealing full solutions
   - Show understanding of the user's situation (listen and reflect)
   - Use storytelling to illustrate impact in a relatable way
   - Be concise and readable (1-2 lines per factor)
   - Never use sales language like "subscribe" or "upgrade"—focus purely on insight and understanding
   
   Include risks such as:
   - High DOM concerns (if applicable, reference exact days and explain why extended time on market creates buyer hesitation)
   - Pricing issues (if applicable, reference price per sqft or comparison, and explain how pricing perception affects buyer interest)
   - Market challenges specific to this property type/location (explain how market dynamics create obstacles)
   - Property-specific obstacles (explain how these unique challenges impact the sale)
   
   Example tone: "Property has been on market 45 days—buyers start questioning what others see that they don't. This subtle doubt can slow momentum even when the property itself is solid."
   
   Can be empty array [] if no significant risks exist.
...

{
  ...
  "riskFactors": [
    "Specific risk factor 1 with data references. Brief explanation of why this matters and how it influences outcomes—framed with empathy and insight.",
    "Specific risk factor 2 with data references. Brief explanation that helps the user understand the impact in a relatable, human way."
  ],
